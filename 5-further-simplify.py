...
bonded_groups_1 = kp.load_diagram_sets("non_unique_knots_4.txt")
bonded_groups_2_base = []

print("Reducing")
for group in kp.bar(bonded_groups_1):
    non_unique_group = set(kp.reduce_equivalent_diagrams(group, depth=3, flype=True))
...
kp.save_diagrams("unique_knots_5.txt", unique_4)
kp.export_pdf(unique_3, "unique_5.pdf")
#kp.export_pdf(unique_5_composite, "unique_3_composite.pdf")
kp.save_diagram_sets("non_unique_knots_5.txt", non_unique_4)

import os

os.makedirs("slike_3", exist_ok=True)
for i, group in enumerate(non_unique_4):
    kp.export_pdf(group, "slike_3\\3_group_" + str(i)  +".pdf")
